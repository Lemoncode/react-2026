...
Vamos a por la página de reservas vamos a dar un primer paso, armar la página con los parámetros adecuados, y mostrar por consola la selección.
...
```
/grill-me Como siguiente paso quiero validar el formulario, vamos a empezar fácil, de primeras vamos a validar que el nombre esté bien informado, vamos a usar TanStack Form y Zod para esto, y vamos a mostrar un mensaje de error debajo del campo de nombre si el nombre no es correcto, el mensaje de error debe desaparecer cuando el usuario corrija el error.

Tan Stack Form obliga a crear unos wrappers alrededor de los componentes, no quiero ir repetiendolos, para ello vamos a crear dentro de src/components una carpeta que llamaremos form donde vamos a poner los wrappers por ejemplo de el input del campo nombre
```
...
Además cuando se pulse en el botón de solicitar reserva, si el formulario no es correcto, tiene que aparecer un aviso para que chequee los campos, acuerdate que estamos usando shadcn
```

Como último paso, si el formulario es correcto vamos a usar Resend para enviar un correo al dueño de la propiedad con los datos de la reserva, para ello vamos a crear una función en el backend que se encargue de enviar el correo, y vamos a llamar a esa función desde el frontend cuando se pulse en el botón de solicitar reserva, pasando los datos del formulario como parámetros.
